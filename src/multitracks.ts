...
class MultiTracks extends EventEmitter<MultiTracksEvents> {
	private tracks: MultiTracksTracks;
	private options: MultiTracksOptions;
	...
	static create(
		tracks: MultiTracksTracks,
		options: MultiTracksOptions,
	): MultiTracks {
		...
		this.initAllAudios().then((durations) => {
			...
			this.rendering.containers.forEach(({ container, wrapper }, index) => {
				...
				container.addEventListener("click", (e) => {
					...
					const time =
						position * durations[index] + tracks[index].startPosition;
			...
			ws.once("decode", () => {
				...
				if (track.intro) {
					...
					if (track.intro.color) {
						(
							introRegion.element.querySelector(
								'[data-resize="right"]',
							) as HTMLElement
						).style.borderColor = track.intro.color;
					}
	...
	private updatePosition(time: number, autoCenter = false) {
		...
		if (time !== this.currentTime) {
			...
			this.rendering.containers.forEach((container, i) => {
				this.rendering.updateCursor(
					container,
					(time - this.tracks[i].startPosition) / this.durations[i],
					autoCenter,
				);
			});
		}
	...
	private onDrag(index: number, delta: number) {
		...
		const mainIndex = this.tracks.findIndex(
			(item) => item.url && !item.draggable,
		);
		const mainTrack = this.tracks[mainIndex];
		const minStart =
			(mainTrack ? mainTrack.startPosition : 0) - this.durations[index];
...
function initRendering(tracks: MultiTracksTracks, options: MultiTracksOptions) {
	let pxPerSec = 0;
	...
	const containers = tracks.map((track, index) => {
		...
		scroll.setAttribute(
			...
			`width: 100%; overflow-x: ${
				track.hideScrollbar ? "hidden" : "scroll"
			}; overflow-y: hidden; user-select: none; position: relative;`,
		);
		...
		cursor.setAttribute(
			"style",
			"height: 100%; position: absolute; z-index: 10; top: 0; left: 0",
		);
	...
	return {
		...
		setMainWidth: (trackDurations: number[], maxDuration: number) => {
			...
			durations.forEach((_, i) => {
				pxPerSec = Math.max(
					...
					containers[i].wrapper.clientWidth / maxDuration,
				);
		...
		updateCursor: (
			{ cursor, scroll }: MultiTracksContainer,
			position: number,
			autoCenter: boolean,
		) => {
